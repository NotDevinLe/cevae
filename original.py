...
for i, (train, valid, test, contfeats, binfeats) in enumerate(
    ...
    with tf.Graph().as_default():
        ...
        x_ph_bin = tf.placeholder(tf.float32, [M, len(binfeats)], name="x_bin")
        x_ph_cont = tf.placeholder(tf.float32, [M, len(contfeats)], name="x_cont")
        t_ph = tf.placeholder(tf.float32, [M, 1])
        y_ph = tf.placeholder(tf.float32, [M, 1])
        x_ph = tf.concat([x_ph_bin, x_ph_cont], 1)
        activation = tf.nn.elu
        z = Normal(
            loc=tf.zeros([tf.shape(x_ph)[0], d]), scale=tf.ones([tf.shape(x_ph)[0], d])
        )
        hx = fc_net(
            z, (nh - 1) * [h], [], "px_z_shared", lamba=lamba, activation=activation
        )
        logits = fc_net(
            hx,
            [h],
        ...
        mu2_t0 = fc_net(
            z, nh * [h], [[1, None]], "py_t0z", lamba=lamba, activation=activation
        )
        mu2_t1 = fc_net(
            z, nh * [h], [[1, None]], "py_t1z", lamba=lamba, activation=activation
        )
        y = Normal(loc=t * mu2_t1 + (1.0 - t) * mu2_t0, scale=tf.ones_like(mu2_t0))
        logits_t = fc_net(
            x_ph, [d], [[1, None]], "qt", lamba=lamba, activation=activation
        )
        qt = Bernoulli(logits=logits_t, dtype=tf.float32)
        hqy = fc_net(
            x_ph, (nh - 1) * [h], [], "qy_xt_shared", lamba=lamba, activation=activation
        )
        mu_qy_t0 = fc_net(
            hqy, [h], [[1, None]], "qy_xt0", lamba=lamba, activation=activation
        )
        mu_qy_t1 = fc_net(
            hqy, [h], [[1, None]], "qy_xt1", lamba=lamba, activation=activation
        )
        qy = Normal(
            loc=qt * mu_qy_t1 + (1.0 - qt) * mu_qy_t0, scale=tf.ones_like(mu_qy_t0)
        )
        ...
        qz = Normal(
            loc=qt * muq_t1 + (1.0 - qt) * muq_t0,
            scale=qt * sigmaq_t1 + (1.0 - qt) * sigmaq_t0,
        )
        ...
        y_post_eval = ed.copy(
            y, {z: qz.mean(), qt: t_ph, qy: y_ph, t: t_ph}, scope="y_post_eval"
        )
        x1_post_eval = ed.copy(
            x1, {z: qz.mean(), qt: t_ph, qy: y_ph}, scope="x1_post_eval"
        )
        x2_post_eval = ed.copy(
            x2, {z: qz.mean(), qt: t_ph, qy: y_ph}, scope="x2_post_eval"
        )
        t_post_eval = ed.copy(
            t, {z: qz.mean(), qt: t_ph, qy: y_ph}, scope="t_post_eval"
        )
        logp_valid = tf.reduce_mean(
            tf.reduce_sum(
                y_post_eval.log_prob(y_ph) + t_post_eval.log_prob(t_ph), axis=1
            )
        ...
        (n_epoch, n_iter_per_epoch, idx) = (
            args.epochs,
            10 * int(xtr.shape[0] / 100),
            np.arange(xtr.shape[0]),
        )
        (tr0, tr1) = (np.zeros((xalltr.shape[0], 1)), np.ones((xalltr.shape[0], 1)))
        (tr0t, tr1t) = (np.zeros((xte.shape[0], 1)), np.ones((xte.shape[0], 1)))
        f1 = {
            x_ph_bin: xalltr[:, 0 : len(binfeats)],
            x_ph_cont: xalltr[:, len(binfeats) :],
            t_ph: tr1,
        }
        f0 = {
            x_ph_bin: xalltr[:, 0 : len(binfeats)],
        ...
        for epoch in range(n_epoch):
            ...
            for j in range(n_iter_per_epoch):
                ...
                info_dict = inference.update(
                    feed_dict={
                        x_ph_bin: x_train[:, 0 : len(binfeats)],
            ...
            if epoch % args.earl == 0 or epoch == n_epoch - 1:
                ...
                if logpvalid >= best_logpvalid:
                    print(
                        "Improved validation bound, old: {:0.3f}, new: {:0.3f}".format(
            ...
            if epoch % args.print_every == 0:
                ...
                print(
                    "Epoch: {}/{}, log p(x) >= {:0.3f}, ite_tr: {:0.3f}, ate_tr: {:0.3f}, pehe_tr: {:0.3f}, rmse_f_tr: {:0.3f}, rmse_cf_tr: {:0.3f}, ite_te: {:0.3f}, ate_te: {:0.3f}, pehe_te: {:0.3f}, dt: {:0.3f}".format(
                        epoch + 1,
                        n_epoch,
                        avg_loss,
                        score_train[0],
        ...
        print(
            "Replication: {}/{}, tr_ite: {:0.3f}, tr_ate: {:0.3f}, tr_pehe: {:0.3f}, te_ite: {:0.3f}, te_ate: {:0.3f}, te_pehe: {:0.3f}".format(
                i + 1,
                args.reps,
                score[0],
                score[1],
...
print(
    "train ITE: {:.3f}+-{:.3f}, train ATE: {:.3f}+-{:.3f}, train PEHE: {:.3f}+-{:.3f}".format(
        means[0], stds[0], means[1], stds[1], means[2], stds[2]
    )
)
(means, stds) = (np.mean(scores_test, axis=0), sem(scores_test, axis=0))
print(
    "test ITE: {:.3f}+-{:.3f}, test ATE: {:.3f}+-{:.3f}, test PEHE: {:.3f}+-{:.3f}".format(
        means[0], stds[0], means[1], stds[1], means[2], stds[2]
    )
)
