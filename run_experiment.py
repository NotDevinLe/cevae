...
OUTCOME_TYPE = {"synthetic": "binary", "ihdp": "continuous", "jobs": "binary"}
...
def run(
    ...
    *,
    ...
    scores = {
        m: {"train": np.zeros((replications, 4)), "test": np.zeros((replications, 4))}
...
def run_jobs(
    ...
    *,
    ...
    scores = {
        m: {"train": np.zeros((replications, 3)), "test": np.zeros((replications, 3))}
        for m in model_names
    }
    for i, (train, valid, test, contfeats, binfeats) in enumerate(
        ...
        for name, model in models.items():
            ...
            if verbose:
                s = scores[name]["test"][i]
                print(
                    f"[{name:8s}] R_pol: {s[0]:.3f}  ATE: {s[1]:.3f}  ATT: {s[2]:.3f}"
                )
    return scores
...
def print_table(scores):
    col_w = 14
    name_w = 10
    header = f"{'Method':<{name_w}s} | {'In-sample':^{2 * col_w + 1}s} | {'Out-of-sample':^{2 * col_w + 1}s}"
    sub = f"{'':>{name_w}s} | {'e_ATE':>{col_w}s} {'sqrtPEHE':>{col_w}s} | {'e_ATE':>{col_w}s} {'sqrtPEHE':>{col_w}s}"
    width = len(header)
    sep = "-" * width
    print(f"\n{sep}")
    print(header)
    ...
    for name, d in scores.items():
        ...
        print(
            f"{label:<{name_w}s} | {_fmt(tr, 1):>{col_w}s} {_fmt(tr, 2):>{col_w}s} | {_fmt(te, 1):>{col_w}s} {_fmt(te, 2):>{col_w}s}"
        )
    print(sep)
...
def print_jobs_table(scores):
    col_w = 14
    name_w = 10
    header = f"{'Method':<{name_w}s} | {'In-sample':^{3 * col_w + 2}s} | {'Out-of-sample':^{3 * col_w + 2}s}"
    sub = f"{'':>{name_w}s} | {'R_pol':>{col_w}s} {'e_ATE':>{col_w}s} {'e_ATT':>{col_w}s} | {'R_pol':>{col_w}s} {'e_ATE':>{col_w}s} {'e_ATT':>{col_w}s}"
    width = len(header)
    sep = "-" * width
    print(f"\n{sep}")
    print(header)
    ...
    for name, d in scores.items():
        ...
        label = DISPLAY_NAMES.get(name, name)
        print(
            f"{label:<{name_w}s} | {_fmt(tr, 0):>{col_w}s} {_fmt(tr, 1):>{col_w}s} {_fmt(tr, 2):>{col_w}s} | {_fmt(te, 0):>{col_w}s} {_fmt(te, 1):>{col_w}s} {_fmt(te, 2):>{col_w}s}"
        )
    print(sep)
...
def save_sweep_plot(
    ...
    *,
    xlabel="X",
    ylabel="Absolute ATE Error",
    title="Test ATE Error",
    xlog=False,
):
...
def parse_args(argv=None):
    ...
    g.add_argument("--wd", type=float, default=0.0001)
    ...
    g3.add_argument(
        "--ihdp-path", default="datasets/IHDP/csv", help="Path to IHDP CSV folder"
    )
    g3b = p.add_argument_group("Jobs dataset options")
    g3b.add_argument(
        "--jobs-path", default="datasets/jobs", help="Path to Jobs NPZ folder"
    )
...
def make_dataset(args, n_override=None):
    ...
    if args.dataset == "synthetic":
        ...
        return Jobs(path_data=args.jobs_path, replications=args.replications)
    else:
...
def main(argv=None):
    ...
    if args.sweep_sizes:
        ...
        for n in args.sweep_sizes:
            ...
            print(f"{'=' * 60}")
            ds = SyntheticDataset(n=n, seed=args.seed, replications=args.replications)
            scores = run(
                ds,
                args.models,
                outcome,
