...
class IHDP(object):
    ...
    def __iter__(self):
        for i in range(self.replications):
            data = np.loadtxt(
                self.path_data + "/ihdp_npci_" + str(i + 1) + ".csv", delimiter=","
            )
            (t, y, y_cf) = (
                data[:, 0],
                data[:, 1][:, np.newaxis],
...
class Jobs:
    ...
    def get_train_valid_test(self):
        for i in range(self.replications):
            ...
            train = (x_tr_full[itr], t_tr_full[itr], y_tr_full[itr], e_tr_full[itr])
            valid = (x_tr_full[iva], t_tr_full[iva], y_tr_full[iva], e_tr_full[iva])
...
class SyntheticDataset(object):
    ...
    def __init__(
        ...
        if n is not None:
            ...
            if flip_prob is not None:
                raise ValueError(
                    "Cannot load pre-generated data with flip_prob; pass n= to generate data on the fly."
                )
