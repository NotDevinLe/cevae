...
class _DualHead(nn.Module):
    ...
    def __init__(self, in_dim, hidden_dim, out_dim, act_b=None):
        ...
        self.head_b = (
            nn.Sequential(nn.Linear(hidden_dim, out_dim), act_b)
            if act_b
...
class CEVAE(nn.Module):
    ...
    def __init__(self, n_bin, n_cont, d=20, h=200, nh=3):
        ...
        self.p_x_cont_head = (
            _DualHead(h, h, n_cont, nn.Softplus()) if n_cont > 0 else None
        )
        self.p_t = mlp(d, [h], 1)
        self.p_y0 = mlp(d, [h] * nh, 1)
        self.p_y1 = mlp(d, [h] * nh, 1)
        self.q_t = mlp(x_dim, [d], 1)
    ...
    def encode(self, x, t, y):
        ...
        sig_z = (t * sig_z1 + (1 - t) * sig_z0).clamp(min=0.001)
    ...
    def decode(self, z, t):
        ...
        if self.p_x_cont_head is not None:
            ...
            x_cont_sig = x_cont_sig.clamp(min=0.001)
        else:
    ...
    def predict_y(self, x, n_samples=1):
        ...
        for _ in range(n_samples):
            ...
            sig_z = (qt * sig_z1 + (1 - qt) * sig_z0).clamp(min=0.001)
        ...
        for epoch in range(epochs):
            ...
            if epoch % check_every == 0 or epoch == epochs - 1:
                ...
                if verbose:
                    ...
                    print(
                        f"[CEVAE] Epoch {epoch + 1:3d}: val bound {logp:.3f} (best {best_logp:.3f}){tag}"
                    )
                if improved:
                    best_logp = logp
