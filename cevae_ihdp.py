...
for i, (train, val, test, contfeats, binfeats) in enumerate(dataset.get_train_valid_test()):
    ...
    # xtr is the features
    # ttr is the treatment
    # ytr is the outcome
    # y_cftr is the outcome if you flipped the treatment
    # mu0tr is the expected outcome under no treatment
    # mu1tr is the expected outcome under treatment
    (xtr, ttr, ytr), (y_cftr, mu0tr, mu1tr) = train
    (xva, tva, yva), (y_cfva, mu0va, mu1va) = val
    (xte, tte, yte), (y_cfte, mu0te, mu1te) = test

    # binfeats is an array of integers denoting the columns of binary features
    # we make sure to put all binary features at the beginning of array
    perm = binfeats + contfeats
    xtr, xva, xte = xtr[:, perm], xva[:, perm], xte[:, perm]
    n_bin, n_cont = len(binfeats), len(contfeats)

    # We need to evaluate our model
    # the evaluator takes in all the data so we have to concatenate
    # the training and validation samples into one
    ...
    for epoch in range(epochs):
        ...
        if epoch % print_every == 0:
            ...
            rmses_tr = eval_tr.y_errors(y0, y1)
    ...
    # final evaluation with L=100 samples
